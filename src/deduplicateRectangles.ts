 ...
 * @param rectangles rectangles to be de-duplicated
 * @returns {DOMRect[]} New list of rectangles
 */
const deduplicateRectangles = (rectangles: DOMRectList | undefined): DOMRect[] => {
  ...
  if (rectangles) {
    for (let i = 0; i < rectangles.length; i++) {
      ...
      } else if (result.some((prevRect): boolean => prevRect &&
        prevRect.y - 4 <= rect.y &&
        prevRect.x - 4 <= rect.x &&
